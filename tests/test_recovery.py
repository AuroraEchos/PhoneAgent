...
def test_back_is_never_blindly_retried() -> None:
    manager = RecoveryManager(RecoveryConfig(max_attempts_per_failure=2, retry_delay_seconds=0))
...
def test_success_ends_failure_episode() -> None:
    manager = RecoveryManager(RecoveryConfig(max_attempts_per_failure=1, retry_delay_seconds=0))
