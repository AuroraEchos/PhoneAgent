...
const translations = {
  en: {
    ...
    "hero.eyebrow": "Open source · v0.1.3 Alpha",
    ...
    "features.resolver.title": "Lazy deterministic app launch",
    "features.resolver.description": "Resolve a model-issued Launch through built-in aliases or an explicit package, check installation on demand, start it through ADB, and verify the foreground package.",
    ...
    "architecture.description": "Every task follows the same observe, plan, execute, verify, and recover loop. Launch resolution happens only when the planner requests it, without startup catalog scans or prompt app context.",
    ...
    "trajectory.release": "Read the v0.1.3 release notes",
    ...
    "limitations.title": "Honest about what v0.1.3 can prove.",
  ...
  zh: {
    ...
    "features.resolver.title": "按需确定性启动应用",
    "features.resolver.description": "模型输出 Launch 后，运行时才通过内置别名或准确 package 解析目标、检查安装状态、使用 ADB 启动，并验证前台包名。",
    ...
    "architecture.description": "所有任务都进入同一条观察、规划、执行、验证与恢复闭环。只有模型请求 Launch 时才解析应用，不在启动阶段扫描目录或注入应用上下文。",
    ...
    "trajectory.release": "阅读 v0.1.3 发布说明",
    "roadmap.kicker": "开发路线",
    ...
    "limitations.title": "如实表达 v0.1.3 能够证明什么。",
...
const derivedLinks = {
  ...
  release: `${repositoryUrl}/releases/tag/v0.1.3`,
