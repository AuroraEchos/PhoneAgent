...
const translations = {
  en: {
    ...
    "hero.eyebrow": "Open source · v0.2.1 Research Release",
    "hero.titleLine1": "An Android agent runtime",
    "hero.titleLine2": "that can see what it does.",
    "hero.description": "PhoneAgent is a focused Research and Evaluation Runtime that combines vision-language planning, strict structured actions, Android execution, evidence verification, and minimal bounded recovery into one auditable loop for real devices.",
    ...
    "features.protocol.description": "One canonical action is parsed through AST and validated against an allow-list. JSON, code fences, multiple actions, and incomplete output are rejected; sensitive operations require confirmation.",
    "features.resolver.title": "Entry-app-first launch",
    "features.resolver.description": "After the first observation, launch an explicitly requested entry app before visual planning; resolve aliases lazily, check installation, and verify the foreground package.",
    ...
    "features.recovery.description": "Recovery is limited to replan, reobserve, one safe retry, takeover, or abort. Tap, Type, Swipe, and Back are never blindly replayed.",
    "features.trajectory.title": "Auditable trajectories",
    "features.trajectory.description": "One AgentEvent source feeds callbacks and trajectories, preserving identical steps, timestamps, payloads, phase changes, and execution evidence.",
    "architecture.kicker": "Architecture",
    "architecture.title": "Deterministic where possible. Visual where necessary.",
    "architecture.description": "Every task starts with a trusted observation. Explicit entry apps take a deterministic first-action path; all remaining work follows the plan, execute, verify, and recover loop without catalog scans.",
    "architecture.point1.title": "Lightweight initialization",
    "architecture.point1.description": "Initialize task state and recovery budgets without enumerating or caching installed applications.",
    "architecture.point2.title": "Entry app before vision",
    "architecture.point2.description": "Resolve an explicit entry app as the first device action; request one model action only when visual planning is needed.",
    ...
    "docs.kicker": "Technical documentation",
    "docs.title": "Read the runtime from its source boundaries.",
    "docs.description": "Eleven Chinese code-guided documents connect entry, observation, planning, actions, safety, verification, recovery, evidence, and the Web Console into one traceable execution path.",
    "docs.meta.documents": "Subsystem documents",
    "docs.meta.lines": "Lines of guidance",
    "docs.meta.path": "Connected runtime",
    "docs.open": "Open the source guide (中文)",
    "docs.stage.foundation": "Runtime foundation",
    "docs.stage.foundation.desc": "Start, observe, and prepare model context.",
    "docs.stage.loop": "Core agent loop",
    "docs.stage.loop.desc": "Coordinate state and turn intent into actions.",
    "docs.stage.reliability": "Safety & reliability",
    "docs.stage.reliability.desc": "Guard execution before and after device side effects.",
    "docs.stage.evidence": "Evidence & interface",
    "docs.stage.evidence.desc": "Persist what happened and expose it to people.",
    "docs.item.entry": "Entry & Configuration",
    "docs.item.device": "Device & Observation",
    "docs.item.model": "Model & Context",
    "docs.item.agent": "Agent Runtime & State",
    "docs.item.action": "Action Subsystem",
    "docs.item.semantic": "Semantic Review",
    "docs.item.freshness": "Freshness Guard",
    "docs.item.verification": "Verification",
    "docs.item.recovery": "Recovery",
    "docs.item.observability": "Observability & Evaluation",
    "docs.item.web": "Local Web Console",
    ...
    "quickstart.step3.description": "Run a multi-step WeChat and Meituan task. The runtime classifies the order task from the original instruction, reviews coordinate actions, and requests confirmation when needed.",
    ...
    "trajectory.description": "A run is not just terminal output. PhoneAgent records phase-change events, model interaction, structured actions, execution results, verification evidence, recovery decisions, and the final outcome.",
    ...
    "trajectory.release": "Read the v0.2.1 release notes",
    "roadmap.kicker": "Roadmap",
    "roadmap.title": "The runtime is small by design. The direction is not.",
    "roadmap.description": "v0.2.1 closes semantic review and Web hardening on the refactor baseline; the next phase freezes the real-device evaluation baseline.",
    "roadmap.complete": "Current release",
    "roadmap.next": "Next",
    "roadmap.future": "Longer term",
    "roadmap.item1.title": "Vision agent loop",
    "roadmap.item1.description": "One live AgentState and one auditable AgentEvent stream.",
    "roadmap.item2.title": "Lazy Android app launch",
    "roadmap.item2.description": "Static aliases, installation checks, package launch, foreground verification.",
    "roadmap.item3.title": "Safe action runtime",
    "roadmap.item3.description": "Strict protocol, confirmation, minimal bounded recovery.",
    "roadmap.web.title": "Local Web Console",
    "roadmap.web.description": "Session checks, live events, prompts, and trajectory inspection.",
    "roadmap.item4.title": "Isolated semantic review",
    "roadmap.item4.description": "Fresh completion review plus coordinate-action review for high-consequence tasks and explicit boundaries.",
    "roadmap.item5.title": "Frozen evaluation baseline",
    "roadmap.item5.description": "Versioned tasks, initial states, device/model matrix, repetitions, external labels, and cost reporting.",
    ...
    "limitations.title": "Honest about what current main can prove.",
    ...
    "limitations.item2.title": "Completion review is not benchmark truth",
    "limitations.item2.description": "A fresh isolated model review gates planner completion, but published task correctness still requires an external annotation or deterministic judge.",
    ...
    "footer.sourceGuide": "Source Guide",
  ...
  zh: {
    ...
    "features.protocol.description": "系统只通过 AST 解析一个规范动作并完成白名单校验；JSON、代码块、多个动作和残缺输出都会被拒绝，敏感操作必须由用户确认。",
    "features.resolver.title": "入口应用优先启动",
    "features.resolver.description": "首次观察后先确定性启动任务明确要求的入口应用；别名仍按需解析，并检查安装状态、使用 ADB 启动和验证前台包名。",
    ...
    "features.recovery.description": "恢复只包含重新规划、重新观察、一次安全重试、人工接管或终止；Tap、Type、Swipe、Back 不会被盲目重放。",
    "features.trajectory.title": "可审计执行轨迹",
    "features.trajectory.description": "回调与轨迹共享同一个 AgentEvent，确保步数、时间戳、payload、阶段迁移与执行证据完全一致。",
    "architecture.kicker": "系统架构",
    "architecture.title": "能够确定时走系统路径，必须理解时再使用视觉。",
    "architecture.description": "所有任务先完成可信观察；明确入口应用走确定性的首动作路径，其余工作继续进入规划、执行、验证与恢复闭环，且不扫描应用目录。",
    "architecture.point1.title": "轻量任务初始化",
    "architecture.point1.description": "只初始化任务状态与恢复预算，不枚举或缓存设备上的已安装应用。",
    "architecture.point2.title": "入口应用先于视觉规划",
    "architecture.point2.description": "明确入口应用先作为首个设备动作启动；只有需要视觉理解时才请求一个模型动作。",
    "architecture.point3.title": "证据与恢复",
    "architecture.point3.description": "验证动作结果，再决定继续、重新规划，或在预算内仅重试安全动作。",
    "architecture.read": "阅读完整架构说明",
    "docs.kicker": "技术文档",
    "docs.title": "沿着源码边界，真正读懂整个运行时。",
    "docs.description": "十一篇严格依照当前代码编写的文档，把入口、观测、规划、动作、安全、验证、恢复、证据与 Web 控制台串成一条可追踪的执行链路。",
    "docs.meta.documents": "篇子系统文档",
    "docs.meta.lines": "行源码导读",
    "docs.meta.path": "条完整运行链路",
    "docs.open": "打开源码导读",
    "docs.stage.foundation": "运行基础",
    "docs.stage.foundation.desc": "启动任务、观察设备并准备模型上下文。",
    "docs.stage.loop": "Agent 核心循环",
    "docs.stage.loop.desc": "协调状态，并把模型意图变成规范动作。",
    "docs.stage.reliability": "安全与可靠性",
    "docs.stage.reliability.desc": "在设备副作用发生前后建立保护边界。",
    "docs.stage.evidence": "证据与外部界面",
    "docs.stage.evidence.desc": "保存运行事实，并把它呈现给使用者。",
    "docs.item.entry": "入口与配置",
    "docs.item.device": "设备与观测",
    "docs.item.model": "模型与上下文",
    "docs.item.agent": "Agent 编排与状态",
    "docs.item.action": "动作子系统",
    "docs.item.semantic": "语义复核",
    "docs.item.freshness": "执行前新鲜度",
    "docs.item.verification": "动作效果验证",
    "docs.item.recovery": "失败恢复",
    "docs.item.observability": "可观测性与评估",
    "docs.item.web": "本地 Web 控制台",
    ...
    "quickstart.step3.description": "运行微信与美团的多步骤任务；运行时会从原始任务识别下单风险、复核坐标动作，并在需要时请求人工确认。",
    "quickstart.copy": "复制",
    "quickstart.copied": "已复制",
    "quickstart.result": "ADB 预检通过 · 模型 API 已响应 · 任务已启动",
    "trajectory.kicker": "执行可追溯",
    "trajectory.title": "关键运行时状态都会留下证据。",
    "trajectory.description": "一次运行不只是终端输出。PhoneAgent 会记录阶段迁移事件、模型交互、结构化动作、执行结果、验证证据、恢复决策以及最终状态。",
    ...
    "trajectory.release": "阅读 v0.2.1 发布说明",
    "roadmap.kicker": "开发路线",
    "roadmap.title": "核心运行时刻意保持简单，但发展方向并不狭窄。",
    "roadmap.description": "v0.2.1 在重构基线上完成语义复核与 Web 加固；下一阶段进入冻结真机评测基线。",
    "roadmap.complete": "当前版本",
    "roadmap.next": "下一阶段",
    "roadmap.future": "长期研究",
    "roadmap.item1.title": "视觉 Agent Loop",
    "roadmap.item1.description": "唯一实时 AgentState 与唯一可审计 AgentEvent 事件流。",
    "roadmap.item2.title": "Android 应用按需启动",
    "roadmap.item2.description": "静态别名、安装检查、package 启动与前台验证。",
    "roadmap.item3.title": "安全动作运行时",
    "roadmap.item3.description": "严格协议、敏感确认和最小有界恢复。",
    "roadmap.web.title": "本地 Web Console",
    "roadmap.web.description": "会话预检、实时事件、交互确认和轨迹检查。",
    "roadmap.item4.title": "隔离语义复核",
    "roadmap.item4.description": "新截图完成复核，以及高后果任务与显式边界下的坐标动作风险复核。",
    "roadmap.item5.title": "冻结评测基线",
    "roadmap.item5.description": "版本化任务、初始状态、设备/模型矩阵、重复次数、外部标注与成本报告。",
    ...
    "limitations.title": "如实表达当前 main 能够证明什么。",
    ...
    "limitations.item2.title": "完成复核不等于评测真值",
    "limitations.item2.description": "规划模型的完成声明会经过新截图隔离复核，但公开任务正确率仍需外部标注或确定性评审器。",
...
const derivedLinks = {
  ...
  release: `${repositoryUrl}/releases/tag/v0.2.1`,
